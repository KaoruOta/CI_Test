...
void Ang_Robo::init() {
  ...
  //180908----
  prev_omega           = 0;
  prev_right_wheel_enc = 0;
  prev_left_wheel_enc  = 0;

  correct_omega           = 0;
  correct_right_wheel_enc = 0;
  correct_left_wheel_enc  = 0;
  //----180908
...
void Ang_Robo::setCommand(float velocity, int forward, float yawratecmd, signed int tail_ang_req, float yawrate,  bool forward_curve_mode, bool tail_stand_mode, bool tail_lug_mode, bool gyro_offset_en, int gyro_offset_value, bool pwm_ctl_en, int pwm_ctl_value) {
  mVelocity           = velocity;
  mForward            = forward;
  ...
  mGyro_offset_en     = gyro_offset_en;
  mGyro_offset_value  = gyro_offset_value;
  mPWM_ctl_en         = pwm_ctl_en;
  mPWM_ctl_value      = pwm_ctl_value;
}
...
void Ang_Robo::run() {
    ...
    case BALANCE_RUN:
      ...
      right_wheel_enc = right_wheel_enc + correct_right_wheel_enc;
      left_wheel_enc  = left_wheel_enc  + correct_left_wheel_enc;
	...
	mForward = mTarget_forward;
      ...
      if(mGyro_offset_en){
	omega = omega + mGyro_offset_value;
      }
      ...
      if(mPWM_ctl_en){
	mLeftWheel.setPWM(mPWM_ctl_value);
	mRightWheel.setPWM(mPWM_ctl_value);
      }else{
	mLeftWheel.setPWM(mBalancer->getPwmLeft());
	mRightWheel.setPWM(mBalancer->getPwmRight());
      }
	...
	//180908----
	prev_omega           = omega;
	prev_right_wheel_enc = right_wheel_enc;
	prev_left_wheel_enc  = left_wheel_enc;
	//----180908
      }
      break;
...
void Ang_Robo::tail_stand_from_balance(){
  ...
  case STAND_VERT:
    ...
    mTurn    = 0;
    balance_off_en = true;
    if(mTail_Motor.getCount() >= 92){ //ikegami changed all -3
      tail_control(93);
      clock_start = robo_Clock->now();
      STAND_MODE = STAND_TO_BALANCE;
    }

    if(mTail_Motor.getCount() < 93){
  ...
  case TAIL_FOR_RUN:
    ...
    if(mTail_Motor.getCount() >=  94){
      ...
      //      correct_omega           = prev_omega - omega;
      correct_right_wheel_enc = prev_right_wheel_enc - right_wheel_enc;
      correct_left_wheel_enc  = prev_left_wheel_enc  - left_wheel_enc;
      //----180908
    }
    break;
