...
//Date:2018.4.27
//Author:Kaoru Ota
//https://github.com/KaoruOta/anago2018_right
...
static void sys_initialize() {
  ...
  ev3_lcd_draw_string("CS_RIGHT_10",0, 40);
  //**********************************************************************************//
  ...
  ev3_lcd_fill_rect(0, 0, EV3_LCD_WIDTH, EV3_LCD_HEIGHT, EV3_LCD_WHITE);
  ev3_lcd_set_font(EV3_FONT_MEDIUM);
  ev3_lcd_draw_string("2018 RIGHT",0, 0);
...
static void log_dat( ){
  ...
  switch(SYS_MODE){
    case LINE_TRACE:
      ...
      log_dat_02[log_cnt]  = gAng_Eye->velocity;


      log_dat_03[log_cnt]  = gAng_Brain->log_dat;
      log_dat_04[log_cnt]  = gAng_Eye->linevalue;
      ...
      //log_dat_0[log_cnt]  = (int)float_to_int_x1000;

      float_to_int_x1000   = gAng_Eye->ave_angle*1000.0;
      log_dat_05[log_cnt]  = (int)float_to_int_x1000;

      float_to_int_x1000   = gAng_Brain->gMotion_Ctl->ref_angle * 1000.0;
      log_dat_06[log_cnt]  = (int)float_to_int_x1000;

      float_to_int_x1000   = gAng_Eye->ave_wheel_load * 1000.0;
      log_dat_07[log_cnt]  = (int)float_to_int_x1000;

      float_to_int_x1000   = gAng_Brain->gMotion_Ctl->load_01 * 1000.0;
      ...
      */
      ...
      log_dat_00[log_cnt]  = gAng_Eye->clock;
      ...
      log_dat_05[log_cnt]  = gAng_Eye->linevalue;
      ...
      */
      ...
      log_dat_02[log_cnt]  = (int)gAng_Eye->xvalue;
      log_dat_03[log_cnt]  = (int)gAng_Eye->yvalue;
      log_dat_04[log_cnt]  = gAng_Brain->log_dat;
      ...
      */
      ...
      log_dat_01[log_cnt]  = gAng_Brain->gyro_offset_en;
      //      log_dat_02[log_cnt]  = gAng_Brain->falling_seesaw;
      log_dat_02[log_cnt]  = gAng_Eye->linevalue;
      log_dat_03[log_cnt]  = gAng_Robo->log_forward;
      log_dat_04[log_cnt]  = gAng_Eye->velocity;
      log_dat_05[log_cnt]  = gAng_Eye->odo;
      log_dat_06[log_cnt]  = (int)float_to_int_x1000;
      ...
      */
      log_dat_00[log_cnt]  = gAng_Eye->linevalue;
      log_dat_01[log_cnt]  = (int)gAng_Eye->xvalue;
    ...
    case TRACK:
      ...
      log_dat_05[log_cnt]  = gAng_Robo->log_forward;
      log_dat_06[log_cnt]  = gAng_Eye->velocity;
      //      log_dat_07[log_cnt]  = (int)gAng_Eye->ave_velo;
      log_dat_07[log_cnt]  = (int)gAng_Eye->ave_angle_500;
    ...
    case DEBUG:
      ...
      log_dat_00[log_cnt]  = gAng_Brain->log_dat;
      log_dat_01[log_cnt]  = gAng_Eye->odo;
      log_dat_02[log_cnt]  = gAng_Eye->velocity;

      log_dat_03[log_cnt]  = gAng_Eye->pre_velo_0p5sec;
      log_dat_04[log_cnt]  = (int)gAng_Eye->ave_accel;
      log_dat_05[log_cnt]  = gAng_Robo->log_gyro;
      log_dat_06[log_cnt]  = gAng_Robo->log_forward;

      float_to_int_x1000   = gAng_Eye->ave_wheel_load * 1000.0;
      log_dat_07[log_cnt]  = (int)float_to_int_x1000;

      log_dat_08[log_cnt]  = gAng_Robo->log_left_pwm;
      ...
      */
      ...
      log_dat_02[log_cnt]  = gAng_Brain->log_dat;
      log_dat_03[log_cnt]  = gAng_Eye->odo;
      log_dat_04[log_cnt]  = gAng_Robo->log_forward;
      log_dat_05[log_cnt]  = (int)gAng_Eye->ave_velo;
      log_dat_06[log_cnt]  = (int)gAng_Eye->ave_accel;
      log_dat_07[log_cnt]  = gAng_Brain->gyro_offset_value;
      ...
      */
      ...
      /*
      log_dat_00[log_cnt]  = gTailMotor.getCount();
      log_dat_01[log_cnt]  = gAng_Brain->tail_stand_mode;
      log_dat_02[log_cnt]  = gAng_Eye->velocity;
      log_dat_03[log_cnt]  = gAng_Eye->odo;

      log_dat_04[log_cnt]  = gAng_Robo->log_left_pwm;
      log_dat_05[log_cnt]  = gAng_Robo->log_right_pwm;
      log_dat_06[log_cnt]  = gAng_Robo->log_gyro;
      log_dat_07[log_cnt]  = gAng_Robo->log_robo_mode;
      log_dat_08[log_cnt]  = gAng_Robo->log_stand_mode;
      */
#endif
...
#ifdef LOG_SHORT
    case LINE_TRACE:
      //      fprintf(fp_wr, "odo,y,velo,log_dat,line,ave_yaw_angle,ref_angle,load,ref_load\n");

      fprintf(fp_wr, "line,odo,x,y,log,forward,vel,yaw_cmd,abs_anngle\n");

      break;

    case TRACK:
      fprintf(fp_wr, "clock,odo,x,y,velo,line,forward,yaw_rate_x1000,ave_yaw_angle\n");
      //      fprintf(fp_wr, "line,odo,x,y,ans_angle,log_forward,velocity,ave_velo,dansa\n");
      break;

    case DEBUG:
      fprintf(fp_wr, "log,odo,velo,pre_velo,accel,gyro,forward,load, pwm\n");
      //      fprintf(fp_wr, "gyro, mW, odo,seesaw_log,forward,velo,accel,wheel_load, pwm\n");
      break;
...
#ifdef LOG_LONG
    case LINE_TRACE:
      fprintf(fp_wr, "line,x,forward, velocity\n");
      break;

    case TRACK:
      fprintf(fp_wr, "tail_angle,gyro_omega,gyro_angle,robo_forward\n");
      break;

    case DEBUG:
      fprintf(fp_wr, "dansa, rising,falling,forward\n");
#endif
...
void brain_task(intptr_t exinf) {
    ...
    //    wup_tsk(MAIN_TASK);  // バックボタン押下
      ...
      gAng_Brain->run();
      gAng_Robo->setCommand(gAng_Eye->ave_velo,//gAng_Eye->velocity, 20181014
			    gAng_Brain->forward,
			    ...
			    //			    gAng_Brain->gyro_offset_plus,
			    gAng_Brain->gyro_offset_en,
			    //			    gAng_Brain->falling_seesaw,
			    //			    gAng_Brain->hilling_seesaw);
			    gAng_Brain->gyro_offset_value,
			    gAng_Brain->pwm_ctl_en,
			    gAng_Brain->pwm_ctl_value);
    }
    ext_tsk();
}
